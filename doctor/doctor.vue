          ...
          <FormItem label="部门" >
             <Row>
                <Col span="12" style="padding-right:10px">
                    <Select v-model="coursesList.name"  filterable>
                        <Option v-for="item in coursesList"  :value.number="item.id" :key="item.id" v-model="coursesList.name">{{ item.name }}</Option>
                    </Select>
                </Col>
            </Row>
          </FormItem>
        <FormItem label="部门说明" v-if='addUser'>
           <Input v-model="formData.remark" placeholder="部门说明" :maxlength="11"></Input>
       	</FormItem>
      </Form>
      <div slot="footer">
...
import { getAllDoctor,doctorTableData,addDoctor,updateDoctor,commentDel} from '@/api/data'
export default {
  data () {
    return {
    	startTime: '', // 日期初始化
              ...
              h('Button', {
                ...
                on: {
                  click: () => {
                    ...
                    getAllDoctor(this.currentPage,this.pageSize).then(res => {
				        if(res.data.success){
  ...
  methods: {
    async pageInit(){
      doctorTableData(this.pageSize,this.currentPage).then(res => {
        if(res.data.success){
   ...
   /* closeUpload(){
    ...
    saveItemOk(){
      var that = this;
      //var DoctorDto = this.formData;
      //console.log(JSON.stringify(DoctorDto))
      if(!DoctorDto.name){
        this.$Message.warning({
          content:'请填写您的姓名！'
        });
...
//      });
//      return;
//    }else if (!/^1(3|4|5|6|7|8)\d{9}$/.test(DoctorDto.phone)){
//      this.$Message.warning({
//        content:'请填写正确的手机号'
//      });
//      return;
...
//			remark:formData.remark;
        ...
        updateDoctor(this.DoctorDto).then(res => {
          // var that = this;
          that.saveItemStatu = false;
          if(res.data.success){
            this.$Message.success({
